...
class WordReportGenerator:
    ...
    def opret_samlet_rangering(self, kvalificerede_chauffoerer):
        ...
        intro_run = intro_tekst.add_run(
            "Nedenstående tabel viser den samlede rangering af chauffører baseret på deres "
            "placering i kategorierne: Tomgang, Fartpilot Anvendelse, Brug af Motorbremse og "
            "Påløbsdrift. Den samlede score er summen af placeringerne i hver kategori, "
            "hvor en lavere score er bedre."
        )
    ...
    def opret_data_tabel(self, data, kolonner, titel):
            """Opretter en tabel med specificerede data og kolonner"""
    ...
    def opret_noegletal_tabel(self, data):
    ...
    def generer_rapport(self):
            ...
            try:
                ...
                for chauffoer, distance in kvalificerede_chauffoerer:
                    # Hent alle data for denne chauffør
                    cursor.execute('''
                ...
                # Find måned og år fra database navnet
                db_navn = os.path.basename(self.db_path)
                dele = db_navn.replace('.db', '').split('_')
                maaned = dele[2].capitalize()
                aar = dele[3]
                ...
                # Opret rapporter mappen hvis den ikke eksisterer
